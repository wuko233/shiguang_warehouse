...
async function getSemesterInfo() {
    try {
        // 发送请求获取页面内容
        const response = await fetch('http://jw.imut.edu.cn/academic/student/currcourse/currcourse.jsdo');
...
async function getFirstCourseDate(yearid, termid) {
    try {
        const response = await fetch('http://jw.imut.edu.cn/academic/manager/coursearrange/studentWeeklyTimetable.do', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            body: `yearid=${yearid}&termid=${termid}&whichWeek=1` // 根据表单数据构造请求体
        });
